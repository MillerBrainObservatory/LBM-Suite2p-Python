...
def grid_search(
    ...
    **kwargs,
    ...
    is written and registered once, then reused for all combinations to avoid
    ...
    reader_kwargs : dict, optional
        Keyword arguments passed to mbo_utilities.imread() when loading data.
        Useful for controlling how raw ScanImage TIFFs are read.
    writer_kwargs : dict, optional
    ...
    **kwargs
    ...
    Notes
    -----
    - When grid_params contains only detection parameters (not registration),
      the binary is written once and registration runs once, then detection
      runs for each parameter combination using the same registered binary.
    - Registration parameters include: nonrigid, block_size, maxregshift,
      maxregshiftNR, smooth_sigma, smooth_sigma_time, snr_thresh, 1Preg, etc.
    - Detection parameters include: threshold_scaling, diameter, anatomical_only,
      sparse_mode, spatial_scale, high_pass, cellprob_threshold, etc.
    ...
    >>> import mbo_utilities as mbo
    >>>
    >>> # search detection params on first 1500 frames of plane 7
    >>> lsp.grid_search(
    ...     input_data="D:/data/raw_250k_frames.tif",
    ...
    ... )
    >>>
    >>> # search cellpose parameters using pre-loaded array
    >>> arr = mbo.imread("D:/data/processed.zarr")
    >>> lsp.grid_search(
    ...     input_data=arr,
    ...     save_path="D:/results/cellpose_search",
    ...     planes=1,
    ...     grid_params={
    ...         "anatomical_only": [2, 3],
    ...         "spatial_hp_cp": [0, 0.5],
    ...
    # registration-related parameters that require re-registration when changed
    REGISTRATION_PARAMS = {
    ...
    for plane_idx in planes_to_process:
        ...
        for i, combo in enumerate(param_combos, 1):
            ...
            if not searching_reg_params:
                # reuse base registration - symlink or copy registered binary
                import shutil

                base_reg_file = base_dir / "data.bin"
                combo_reg_file = combo_save_path / "data.bin"

                # copy the registered binary (symlinks don't work well on Windows)
                if base_reg_file.exists() and not combo_reg_file.exists():
                    shutil.copy2(base_reg_file, combo_reg_file)

                # load base ops and merge with defaults (in case saved ops is from older run)
                ...
                combo_ops.update({
                    ...
                    "do_registration": 0,  # skip registration
                    "roidetect": 1,
                })
                ...
                # searching registration params - need full run_plane
                # start with defaults and overlay user ops + combo params
                combo_ops = copy.deepcopy(default_ops())
